...
def load_model(model, network_type, algorithm, appliance, saved_model_dir):
    ...
    # Keras 3 cannot deserialise the metrics stored in legacy .h5 files, so load the
    # model uncompiled and compile it with the loss and metrics used for training.
    model = tf.keras.models.load_model(model_name, compile=False)
    model.compile(loss="mse", metrics=["mse", "msle", "mae"])
