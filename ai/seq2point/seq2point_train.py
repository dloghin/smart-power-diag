...
class Trainer():
    ...
    def __init__(self, appliance, batch_size, crop, network_type, 
                 ...
                 patience=3, min_delta=1e-6, verbose=1, skip_rows_train=10000000, validation_steps=100):
        ...
        self.__skip_rows_train=skip_rows_train
        self.__validation_steps=validation_steps
    ...
    def train_model(self):
        ...
        # A non-positive number of validation steps means validating on every window of the validation set.
        if self.__validation_steps <= 0:
            self.__validation_chunker.check_if_chunking()
            number_of_validation_windows = self.__validation_chunker.total_size - 2 * self.__window_offset
            self.__validation_steps = int(np.ceil(number_of_validation_windows / self.__batch_size))
            print("Validating on all", number_of_validation_windows, "windows in", self.__validation_steps, "steps")
